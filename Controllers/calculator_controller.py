...
class CalculatorController:
    ...
    def __init__(self):
        ...
        self.function_map = {
            'sin': [sine, '\tCalculates the sine of an input x. Optional: Specify "radians" or "degrees" at the end of '
                          'the input to specify the input type. Default is radians if nothing is added.'
            ...
            'log': [log, '\tCalculates the logarithm for an operand x.\n\t\tOptional: base b is optional, b = 10 if it '
            ...
            'stdev': [standard_deviation, 'Calculates the standard deviation for a given input [x,y,z,...0/1]. Optional'
                                          ': Specify "population" or "sample" at the end of the input to specify '
    ...
    def parse_function_and_dispatch(self, function, arguments, precision=0):
        ...
        if function in self.function_map.keys():
            try:
                ...
                self.compute_history.append(ComputeResult(function, arguments, result, False, None, None,
                                                          None, None, None))
            except Exception as e:
                print(str(e))
        else:
            self.compute_history.append(ComputeResult(function, arguments, None, True, "Invalid function name", None,
                                                      None, None, None))

        # Notify the observers that the model has changed
        self.notify()

    def arith_parse(self, function, arguments, precision, function2, arguments2, precision2, operator):
        # Try converting the list of strings into a list of numbers
        result2 = 0
        try:
            arguments = [float(i) for i in arguments]
            ...
            self.compute_history.append(ComputeResult(function, arguments, None, True, "Invalid arguments", None,
                                                      None, None, None))
        if (function in self.function_map.keys()) and (function2 in self.function_map.keys()):
            try:
                result = self.function_map[function][0](arguments)
                # Check if user has specified a precision
            ...
            try:
                ...
                print(str(e))
            self.compute_history.append(ComputeResult(function, arguments, result, False, None, function2,
                                                      arguments2, result2, operator))

        else:
            self.compute_history.append(ComputeResult(function, arguments, None, True, "Invalid function name",
                                                      function2, arguments2, None, operator))
        self.notify()

    def invalid_user_input(self, input):
        self.compute_history.append(ComputeResult(input, None, None, True, "Invalid user input", None,
                                                  None, None, None))
